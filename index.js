		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD Pepyto BOT*',
					benned: 'Você para a banda,  favor, contate o proprietário para abrir sua banda',
					...
					userB: `┄ LISTA 」─┄\nOlá Kak !\nDesculpe, irmão. Você não está registrado como amigo de Pepyto. Registre-se para fazer amizade com o bot Pepyto por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Pepy|\n\n┄Pepy BOT 」─┄`,
...
if (messagesC.includes("lorena")){
	...
	if (messagesC.includes("teamo")){
			client.updatePresence(from, Presence.composing)
			reply("eu também te amo ❤")
	}
...
//msc fim
            ...
            client.sendMessage(from, tod, image, { quoted: mek, caption: 'Digitar oque?.'})                  
	}
...
break
		   ...
		   await limitAdd(sender)
				...
				case 'meme':
					...
					ze =  ri[Math.floor(Math.random() * ri.length)];
					nye = await getBuffer(ze)
					client.sendMessage(from, nye, image, { caption: 'hoje eu tou comédiante︄', quoted: mek })
					await limitAdd(sender) 	
					break 
			    case 'kbbi':
...
// recursos anime, random @Pepyto domina vida
          ...
          reply('Espere..')
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭─┄┄┄*〄 *PREMIUM USER👑* 」\n*┄+ wa.me/551999897566*╰─┄┄┄┄┄*〄 *Pepyto* 〄*┄┄┄┄*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO Pepyto BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'belle1':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '👀︄'})
					break
				case 'lofi':
					memein = await kagApi.memeindo()
				...
				case 'nsfwloli1':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Rum︄'})
					break
				case 'reislin':
					memein = await kagApi.memeindo()
				...
				case 'meme':
					...
					ze =  ri[Math.floor(Math.random() * ri.length)];
					nye = await getBuffer(ze)
					client.sendMessage(from, nye, image, { caption: 'cringe︄', quoted: mek })
					await limitAdd(sender) 	
					break
				case 'memeindo':
					memein = await kagApi.memeindo()
        ...
        case 'promover':
				...
				case 'toimg':
					...
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSa
